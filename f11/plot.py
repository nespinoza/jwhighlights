...
# First, estimate luminosities of the stars (modulo Boltzmann's constant and factor of 4*pi as we'll normalize to the Sun's):
#Lstar = ( ( all_stellar_radii * q.Rsun )**2 ) * ( ( all_stellar_teff * q.K )**4 )
#jwst_Lstar = ( ( jwst_exoplanet_stellar_radii * q.Rsun )**2 ) * ( ( jwst_exoplanet_stellar_teff * q.K )**4 )
#Lsun = ( ( 1. * q.Rsun )**2 ) * ( ( Teff_sun * q.K )**4 )

# Now I/Isun:
#I = ( Lstar.value / Lsun.value ) * ( ( 1. )**2 / ( all_exoplanet_distances )**2 )
#jwst_I = ( jwst_Lstar.value / Lsun.value ) * ( ( 1. )**2 / ( jwst_exoplanet_distances )**2 )
...
#jwst_Ixuv = jwst_I * ( jwst_Lstar.value / Lsun.value )**(-0.6)
...
plt.plot(x,y,'-', color = 'cornflowerblue', lw = 10, alpha = 0.5)

# Plot all the exoplanets:
plt.plot(escape_velocity[idx_all], Ixuv[idx_all], '.', color = 'silver')

# Plot JWST exoplanets:
plt.plot(jwst_escape_velocity[idx_jwst], 
         jwst_Ixuv[idx_jwst],
...
plt.tight_layout()
plt.savefig('espinoza_pre-figure11.pdf', dpi=300)
# Show the plot
plt.show()
